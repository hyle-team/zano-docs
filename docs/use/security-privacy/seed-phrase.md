...
A **passphrase** is an additional layer of security that can be added to your seed phrase. When used together, the seed phrase and passphrase are both required to restore the wallet — one without the other is not enough. Here’s a detailed explanation of how a seed phrase secured with a passphrase works:
...
- Unlike the seed phrase, which is generated by the wallet, the passphrase is chosen by you. It follows the same character and length rules as other Zano passwords (see the [Password Specification](/docs/build/password-specs)).
